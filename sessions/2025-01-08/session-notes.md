...
- **时长**: 约 90 分钟
- **格式**: 深入探讨
- **主要主题**: 反射、单元测试、OpenAPI、RPC、服务雪崩、协程、消息幂等性
...
### 服务雪崩的场景
```
库存服务变慢 → 订单服务线程等待 → 线程池耗尽 → 订单服务挂了 → 依赖它的服务也挂了 → 整个系统崩溃
```
...
| 方式 | 原理 | 优点 | 缺点 |
|------|------|------|------|
| 线程池隔离 | 独立线程池 | 可以超时控制 | 线程切换开销 |
| 信号量隔离 | 计数器限流 | 轻量，无切换 | 不能超时 |

**学生理解**: 信号量隔离像停车场车位，限制并发数
...
### 今日已解决
- ✅ 服务雪崩处理（超时、熔断、线程隔离、降级）
- ✅ 消息幂等性（唯一ID+去重表、数据库唯一约束、Redis SETNX、状态机）
